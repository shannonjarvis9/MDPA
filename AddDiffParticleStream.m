function [ output_args ] = AddDiffParticleStream(num, x0, y0, PartAng, Type, Ep, Seper)
%AddParticleStream(5, 0.1, 10, -pi / 2, 1, Ep * C.q_0, 5);

%Inputs
%num = number of particles in stream
%x0 = 
%y0 = 
% PartAng = angle the particles are launched from 
% Type = used to set the mass of particle 
global AtomSpacing x y AtomType Vx Vy Mass0 Mass1 nAtoms

if Type == 0
    Mass = Mass0;
else
    Mass = Mass1;
end

for p = 0:num - 1
    nAtoms = nAtoms + 1;
    x(nAtoms) = x0 * AtomSpacing - Seper * p * AtomSpacing * cos(PartAng);
    y(nAtoms) = y0 * AtomSpacing - Seper * p * AtomSpacing * sin(PartAng);
    AtomType(nAtoms) = Type;
end

V = sqrt(2 * Ep / Mass);

for p = 1:num
    Vx(nAtoms - num + p) = V * cos(PartAng);
    Vy(nAtoms - num + p) = V * sin(PartAng);
end

end
